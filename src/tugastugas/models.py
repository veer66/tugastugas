...
class Task(Base):
    ...
    due_date: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=False), nullable=True)
    ...
    last_modifier: Mapped["User"] = relationship(foreign_keys=[last_modifier_id])
...
class HTask(Base):
    ...
    operation_executed_at: Mapped[datetime.datetime] = mapped_column(TIMESTAMP(timezone=False), nullable=False, server_default=text('now()'))
    data_after_executed_operation: Mapped[dict] = mapped_column(JSONB, nullable=True)
